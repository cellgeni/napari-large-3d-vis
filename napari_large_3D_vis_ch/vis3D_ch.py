        ...
        if len(str(filepath_tiff))>3:
            ...
            if 'multiscales' in zobj.attrs:
                ...
                if N_ch == 1:
                    ...
                    cmap_list_updated = cmap_label_list[0]
                else:
                    #data = dask.array.from_zarr(zobj)
                    print(data[0])
                    print(data[1])
                    dask_data = []
                    cmap_list_updated = cmap_label_list[:N_ch]
                    for nch in range(N_ch): 
                        dd = []
                        for n_pyr in range(len(data)):
                            d = np.take(dask.array.from_zarr(data[n_pyr]), nch, axis = ax_ch)
                            dd.append(d)
                        dask_data.append(dd)
                
                
            else:
                print('no multiscales')
                lazy_imread = delayed(imread)
